# ------------------------------------------------------
# call DMPs ---  limit to CEU samples. has genetic PC. does not include
# microarray slide as covariate.
# ------------------------------------------------------
rm(list=ls())
require(minfi)
...
outRoot <- rootDir
outDir <- sprintf("%s/dmp_DX_QTL/agesexslidePMI_CEU_PC12_noSlide_%s",outRoot,dt)
# ------------------------------------------------------
...
for (nm in names(dmpFiles)[3]) {
	...
	cat("* Manhattan plot\n")
	pdf(sprintf("%s.manhattan.pdf",sub(".txt","",dmpFiles[[nm]])),
		width=11,height=6)
	tryCatch({
		manhattanPlot(MSet.genome,dmp); 
	},error=function(ex){
		print(ex)
	},finally={
		dev.off()
