...
sigFile <- "/home/shraddhapai/Epigenetics/NARSAD/output_files/MouseRnaseq2/diffEx_180821/gpFX/diffEx.txt"

dt <- format(Sys.Date(),"%y%m%d")
logFile <- sprintf("gsea_FC_%s.log",dt)
...
gsea_cmd <- paste("java -cp ", gsea, " -Xmx8G xtools.gsea.GseaPreranked -gmx ", gmtFile, " -norm meandiv -nperm 1000 -rnk ", rnkFile, " -scoring_scheme classic -rpt_label mouseFC_RNAseq -create_svgs false -make_sets true -plot_top_x 20 -rnd_seed timestamp -set_max 200 -set_min 10 -zip_report false -out ", outDir, " -gui false", sep="")
system(gsea_cmd)
